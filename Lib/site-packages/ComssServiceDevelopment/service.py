...
class ServiceController(object):
    ...
    def start(self):
        ...
        # service in/out
        in_descr = self.service_desc.get("inputs")
        out_descr = self.service_desc.get("outputs")
        if in_descr:
            self.service.declare_inputs()
            for _input_id, _input_params in in_descr.iteritems():
                self.service.inputs[_input_id].set_params(_input_params)
                self.service.inputs[_input_id].init()
        if out_descr:
            self.service.declare_outputs()
            for _output_id, _output_params in out_descr.iteritems():
